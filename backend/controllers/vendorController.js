...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadPath = file.fieldname === 'idDocument' ? 'uploads/verification/id/' : 'uploads/verification/business/';
    cb(null, uploadPath);
  },
  filename: function (req, file, cb) {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const prefix = file.fieldname === 'idDocument' ? 'id' : 'business';
    cb(null, `${prefix}-${req.user.uid}-${uniqueSuffix}${path.extname(file.originalname)}`);
  }
});
...
const fileFilter = (req, file, cb) => {
  ...
  if (file.mimetype.startsWith('image/') || file.mimetype === 'application/pdf') {
    ...
    cb(new Error('Only image files and PDFs are allowed for verification!'), false);
  }
};
...
const SERVICE_CATEGORIES = {
  'Home Services': [
    'Cleaning', 'Plumbing', 'Electrical', 'HVAC', 'Landscaping', 'Pest Control',
    'Home Security', 'Appliance Repair', 'Painting', 'Roofing', 'Carpentry'
  ],
  'Personal Services': [
    'Personal Training', 'Massage Therapy', 'Hair Styling', 'Makeup', 'Tutoring',
    'Pet Care', 'Child Care', 'Elder Care', 'Personal Shopping'
  ],
  'Business Services': [
    'Accounting', 'Legal', 'Marketing', 'Web Development', 'Graphic Design',
    'Photography', 'Videography', 'Event Planning', 'Catering', 'Translation'
  ],
  'Automotive': [
    'Auto Repair', 'Car Detailing', 'Towing', 'Mobile Mechanic', 'Car Rental'
  ],
  'Health & Wellness': [
    'Fitness Training', 'Nutrition Counseling', 'Mental Health', 'Physical Therapy',
    'Chiropractic', 'Acupuncture', 'Yoga Instruction'
  ],
  'Technology': [
    'Computer Repair', 'Phone Repair', 'Tech Support', 'Software Development',
    'Data Recovery', 'Network Setup', 'Smart Home Installation'
  ]
};
...
const updateVendorSkills = async (req, res) => {
  try {
    ...
    for (const skill of skills) {
      if (!skill.category || !skill.subcategories || !Array.isArray(skill.subcategories)) {
        return res.status(400).json({ error: 'Invalid skill format' });
      }
      
      if (!SERVICE_CATEGORIES[skill.category]) {
        return res.status(400).json({ error: `Invalid category: ${skill.category}` });
      }

      for (const subcategory of skill.subcategories) {
        if (!SERVICE_CATEGORIES[skill.category].includes(subcategory)) {
          return res.status(400).json({ error: `Invalid subcategory: ${subcategory}` });
        }
      }
    }

    // Find user first
    let user = await User.findOne({ firebaseUid: req.user.uid, role: 'vendor' });
...
const uploadVerificationDocuments = async (req, res) => {
  try {
    ...
    if (req.files.idDocument && req.files.idDocument[0]) {
      updateFields['vendorInfo.verification.idDocument'] = `/uploads/verification/id/${req.files.idDocument[0].filename}`;
    }
    
    if (req.files.businessLicense && req.files.businessLicense[0]) {
      updateFields['vendorInfo.verification.businessLicense'] = `/uploads/verification/business/${req.files.businessLicense[0].filename}`;
    }
...
const completeOnboarding = async (req, res) => {
  try {
    ...
    const hasSkills = user.vendorInfo.skills && user.vendorInfo.skills.length > 0;
    const hasVerificationSubmitted = user.vendorInfo.verification.status === 'submitted';

    if (!hasSkills) {
      return res.status(400).json({ error: 'Please add at least one skill before completing onboarding' });
    }
